...
import type { ToolType, PointerEventData, Point } from '../types';
...
export abstract class BaseShapeTool extends BaseTool {
  ...
  protected previewCtx: OffscreenCanvasRenderingContext2D;
  ...
  constructor(
    ...
    super(type, label, shortcutKey, layerManager, commandHistory, viewport);
  ...
  onPointerMove(event: PointerEventData): void {
    ...
    this.drawShape(this.previewCtx, this.startPoint, event.point, event.shiftKey);
  }
  ...
  confirmPlacement(): void {
    ...
    if (before) {
      this.commandHistory.push(
        new DrawCommand(layer.id, before, after, this.layerManager, this.label),
      );
    }
