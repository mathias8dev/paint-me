...
export class RoundedRectangleTool extends BaseTool {
  ...
  constructor(
    ...
    super(ToolType.RoundedRectangle, 'Rounded Rectangle', 'u', layerManager, commandHistory, viewport);
  ...
  onPointerMove(event: PointerEventData): void {
    ...
    this.drawRoundedRect(this.previewCtx, rect);
  }
  ...
  confirmPlacement(): void {
    ...
    if (before) {
      this.commandHistory.push(
        new DrawCommand(layer.id, before, after, this.layerManager, 'Rounded Rectangle'),
      );
    }
  ...
  private drawRoundedRect(
    ...
    const radius = Math.min(this.config.cornerRadius, rect.w / 2, rect.h / 2);
